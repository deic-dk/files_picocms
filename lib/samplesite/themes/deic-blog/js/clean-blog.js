...
jQuery(document).ready(function($) {
	...
	// Make pop-out link visible if in an iframe
	
	if(window.location!==window.parent.location){
	// We're in an iframe
		$('.popout').removeClass('hidden');
		$('.avatardiv').parent().hide();
	}
  ...
  /*$.ajax({
        ...
        $(window).on('scroll', {
            ...
            function() {
                ...
                if (currentTop < this.previousTop) {
                    ...
                    if (currentTop > headerHeight+200){
                   	 setTimeout(function(){
    ...
    //Open file in file/editor view
  	$('.edit-button').click(function() {
   		...
   		var id = $('.edit-button').attr('dir_id') || '';
   		var file_id = $('.edit-button').attr('id') || '';
 		var owner = $('.edit-button').attr('owner') || '';
  		var user = $('.edit-button').attr('user') || '';
   		...
   		window.location.href = userhomeurl+'/index.php/apps/files/?mime=text/markdown&dir='+
   		dir+'&group='+group+'&owner='+(owner!=user?owner:'')+'&id='+(owner!=user?id:'')+'&file_id='+file_id+
					...
					function(result) {
						if (result.status === 'success') {
					  	...
					  	$('comment .comment_avatar.avatardiv').first().avatar($('comment .comment_avatar').first().attr('author'), 48);
							...
							alert(t('core', 'Could not create file. ')+result.data.message);
						}
					}
				);
  	});
    ...
    if (e.keyCode == 13) {
			...
			var dirId = $('.write-post').attr('dir_id');
			...
			if(!filename || !filename.length){
				window.location.href = '/index.php/apps/files/?dir='+dir+'&group='+group+'&owner='+owner;
			}
			else{
				// Create file
				$.ajax({
					url: OC.webroot+'/themes/deic_theme_oc7/apps/files/ajax/newfile.php',
					data: {
						//id: sharedDirId,
						id: dirId,
						dir: dir,
					...
					success: function(result) {
						if(result.status==='success' && result.data && result.data.id){
							window.location.href = '/index.php/apps/files/?mime=text/markdown&dir='+dir+'&id='+dirId/*sharedDirId*/+'&group='+group+'&owner='+(sharedDirId?owner:'')+
								'&file='+encodeURIComponent(filename)+'&file_id='+result.data.id;
						}
						else {
							alert((typeof(result.data)!='undefined'?result.data.message:'Error: ')+t('core', 'Could not create file'));
						}
					},
					error: function(result){
						alert((typeof(result.data)!='undefined'?result.data.message:'Error. ')+t('core', 'Could not create file. Try logging in.'));
					}
				});
			}
		}
	...
	$('.modal-image').click(function(ev){
		...
		/*if(img.parents('.post-excerpt').length){
		  return;
		}*/
...
<div class="modal">\
  <div class="imagebox">\
	...
	$('div#toc').first().prepend('<div id="toc_header">Contents <span id="toc_toggle">[show]</span><span>&nbsp;</span><input id="presentation_mode" title="Presentation mode"  type="checkbox"></div>');
	$('div#toc ul').hide();
	$('#toc_toggle').click(function(ev){
		if($('div#toc ul:visible').length){
			$('div#toc ul').hide();
			$('#toc_toggle').text('[show]');
		}
		else{
			$('div#toc ul').show();
			$('#toc_toggle').text('[hide]');
		}
	});
	if($('div#toc').length){
		$('.edit-button').css('right', 'unset');
		$('.edit-button').css('left', '132px');
		$('.edit-button').css('top', '0');
	}

	$('#presentation_mode').click(function(ev){
		if(!$('#presentation_mode').is(':checked')){
			$('.navbar, #show_comments, .foot, footer, header+hr.small, button.edit-button').show();
		}
		else{
			$('.navbar, #show_comments, .foot, footer, header+hr.small, button.edit-button').hide();
		}
		$('h1, h2, h3, h4').each(function(i, el){
			if(!$('#presentation_mode').is(':checked')){
				$(this).show();
				$(this).nextUntil('h1,h2,h3,h4').show();
			}
			else{
				$(this).hide();
				$(this).nextUntil('h1,h2,h3,h4').hide();
			}
		})
		$('h1, h2, h3, h4').prevAll().addBack().each(function(i, el){
			if(!$('#presentation_mode').is(':checked')){
				$(this).show();
				$(this).nextUntil('h1,h2,h3,h4').show();
			}
	...
	$('[href^=#toc_head]').click(function(ev){
		...
		if($('#presentation_mode').is(':checked')){
			$('h1, h2, h3, h4').each(function(i, el){
				var myType = $(this).prop('nodeName');
				$(this).hide();
				$(this).nextUntil(myType).hide();
			});
			var target = $(ev.target).attr('href');
			$(target).show();
			var myType = $(target).prop('nodeName');
			//var replace = '^.*'+myType+'[, ]*';
			var replace = '('+myType+').*';
			var re = new RegExp(replace, 'i')
			var myLevels = 'h1, h2, h3, h4'.replace(re, '$1');
			$(target).nextUntil(myLevels).show();
		}
		else{
			scrollToEl($(ev.target).attr('href'), 0/*-76*/);
		}
	});
